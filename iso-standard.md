...
The `location` element represents spatial\geometric information with 2 dimentional x & y corodinate (and optional resolution) attributes of the format `<location x="integer" y="integer" resolution_x="integer" resolution_y="integer" >` with `0 <= x|y < resolution_x|resolution_y`.

- Single coordinate at (100, 200): `<location x="100" y="200" />`
- Bounding box with (x0, y0) = (100, 200) and (x1, y1) = (300, 400): `<location x="100" y="200"/><location x="300" y="400"/>`
...
| `code lang="string"` | Code block with optional language programming name attribute |
| `picture` | Image or graphic element |
| `signature signed=true\|false` | Signature element, signed=false means it is empty |
| `form` | Form structure |
...
| `formula` | Mathematical expression |
| `code lang="string"` | Code block with optional name of the programming language |
| `picture` | Image or graphic element |
-->
...
```xml
<table_group>
  <caption><location x=x0 y=y0/>...<location x=x1 y=y1/>
    Table 1: Experimental Results
  </caption>
  <otsl><location x=x0 y=y0/>...<location x=x1 y=y1/>
...
```xml
<unordered_list>
  <list_item>location x=x0 y=y0/>...<location x=x1 y=y1/>
    <marker>•</marker>
  ...
  <checkbox selected=true>location x=x0 y=y0/>...<location x=x1 y=y1/>
      Completed task
  </checkbox>
  <checkbox selected=false>location x=x0 y=y0/>...<location x=x1 y=y1/>
      Pending task
  </checkbox>
</unordered_list>
```
